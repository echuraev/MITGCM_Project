...
clear filenames Hr H Lx Ly Lg Lw r2 r1 h2r h1r V dh g nu i ReW T X X_dim Z ...
    c0 u_dim u_diml u_dimh index k nx nz rho_m t t_diml t_dimr t_g x_point ...
    z_points h Z_dim nz_points p dir_name dir_prefix exp_tar Tint ...
    u_dim_int yTick yTickMarks Hl dH DdSurf delta dg dr h1 h1l h2 h2add ...
    h2l r_m zpl zpr;
...
nx = length(X);
nz = length(Z);
%u = nc_varget(filenames{1}, 'U');
T = nc_varget(filenames{1}, 'T')*c0/h2r;
for i=2:length(filenames)
%    u = cat(1, u, nc_varget(filenames{i}, 'U'));
...
for i=1:length(filenames)
    ...
    for p=1:nz_points
        for t=1:size(u, 1)
            u_dim(p, t_g + t - 1) = u(t, z_points(p, 2), x_point(2))/c0;
        end;
    end;
    t_g = t_g + size(u, 1);
    clear u
...
hold off;
axis([t_diml t_dimr u_diml u_dimh]);
xlabel('tc_{0}/h_{2}','FontSize',18);
ylabel('u/c_{0}','FontSize',18);
print(h, '-dtiff', '-r95', '01_horizontal_velocity.tiff');
%* end
